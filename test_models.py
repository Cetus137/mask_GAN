...
def test_models():
    ...
    print("✓ Generator produces 256x256 images")
    
    # Test Discriminator
    print("\nTesting Discriminator...")
    real_images = torch.randn(4, 1, 256, 256, device=device)
    output = netD(real_images)
    print(f"Discriminator output shape: {output.shape}")
    assert len(output.shape) == 2 and output.shape[0] == 4, f"Expected batch dimension 4, got {output.shape}"
    print("✓ Discriminator processes 256x256 images")
    ...
    print("\n✅ All tests passed! Models are ready for 256x256 training.")
